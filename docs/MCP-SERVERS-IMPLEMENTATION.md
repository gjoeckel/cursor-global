...
1. **Configuration**: Defined in `~/.cursor/mcp.json`
2. **Startup**: Cursor IDE reads config on launch
...
The `setup-mcp-servers.sh` script automatically:
1. Copies `config/mcp.json` to `~/.cursor/mcp.json`
2. Expands `${HOME}` to actual home directory path
...
3. Restart Cursor IDE after setting variables
...
3. Check tool count: Run `mcp-health` workflow
...
- [ ] Servers verified (`mcp-health`)

### Next Steps

1. Configure `mcp.json` with your paths
2. Set required environment variables
3. Restart Cursor IDE
4. Verify servers with `mcp-health`
...
- [Cursor Autonomous Setup Guide](./CURSOR-AUTONOMOUS-SETUP.md)
